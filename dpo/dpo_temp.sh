...
# conda activate fairseq2_2025-08-09
...
PROJECT="rlhi_0821"
DATANAME="preference_pairs_0821_synthetic_j1" # no ".jsonl"

# PROJECT="mixed_0710"
# DATANAME="mixed"

DATASET="/home/chuanyang/data/${DATANAME}.jsonl"
...
srun --export=ALL fairseq2 lm preference_finetune --no-sweep-dir /checkpoint/ram/chuanyang/dpo/${RUNNAME} \
--config-file ${CONFIG} --config dataset.path=${DATASET} criterion.config.beta=${BETA} optimizer.config.lr=${LR} regime.checkpoint_every_n_steps=50 lr_scheduler.config.num_warmup_steps=${WARMUP} lr_scheduler.config.final_lr=${FINAL_LR} trainer.grad_accumulation.num_batches=4 optimizer.config.eps=${EPS} common.metric_recorders.wandb.enabled=True common.metric_recorders.wandb.project=${PROJECT} common.metric_recorders.wandb.run_name=${RUNNAME}

# srun --export=ALL fairseq2 lm preference_finetune --no-sweep-dir /checkpoint/ram/chuanyang/dpo/${RUNNAME} \
# --config-file ${CONFIG} --config dataset.path=${DATASET} criterion.config.beta=${BETA} optimizer.config.lr=${LR} regime.checkpoint_every_n_steps=500 regime.checkpoint_every_n_data_epochs=100 lr_scheduler.config.num_warmup_steps=${WARMUP} lr_scheduler.config.final_lr=${FINAL_LR} trainer.gradient_accumulation=4 optimizer.config.eps=${EPS} common.metric_recorders.wandb.enabled=True common.metric_recorders.wandb.project=${PROJECT} common.metric_recorders.wandb.run=${RUNNAME}

# fairseq2 lm preference_finetune --dump-config

# fairseq2 lm preference_finetune --no-sweep-dir /checkpoint/ram/chuanyang/dpo/test --config-file "/home/chuanyang/dpo/dpo_config.yaml" --config dataset.path="/home/chuanyang/data/preference_pairs_0810_3000_rlhi.jsonl" criterion.config.beta=0.1 optimizer.config.lr=7e-7 regime.num_steps=1 regime.checkpoint_every_n_steps=500 regime.checkpoint_every_n_data_epochs=100 lr_scheduler.config.num_warmup_steps=30 lr_scheduler.config.final_lr=1e-15 trainer.grad_accumulation.num_batches=4 optimizer.config.eps=1e-8